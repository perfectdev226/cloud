...
while ($row = $q->fetch_array()) {
	...
	$pages[$row['locale']] = StandardPageManager::getPage('tools', $row['locale']);
}
...
if (isset($_POST['isPostback']) && !DEMO) {
	...
	if (!is_array($tools) || !is_array($categories)) {
		$studio->showFatalError("Failed to parse JSON formatted POST tools or categories.");
	}
	...
	# Translations
	$update = array(
		'' => $studio->basedir . "/resources/bin/en-us/categories.json"
	);

	foreach ($languages as $locale => $name) {
		$update[$locale] = $studio->basedir . "/resources/languages/$locale/categories.json";
	}

	foreach ($update as $locale => $file) {
		...
		foreach ($categories as $index => $catName) {
			$token = $catName['token'];
			$text = $locale === '' ? $catName['token'] : $catName['names'][$locale];
			$items["@$token"] = $text;
		}
		...
		if ($bool === false) {
			$studio->showFatalError("Failed to write to $file. Please try making this directory and all files inside have chmod 0777. Contact support for further assistance.");
		}
	}
	...
	foreach ($categories as $index => $catName) {
		$token = $catName['token'];
		$catDescriptions[$token] = [];

		foreach ($catName['descriptions'] as $descLocale => $descText) {
			$catDescriptions[$token][$descLocale] = $descText;
		}
	}
	$studio->setopt('category-descriptions', json_encode($catDescriptions));
...
foreach ($data as $catName => $tools) {
	if (!in_array($catName, $catData)) {
		foreach ($tools as $id) {
			foreach ($list as $tool) {
				if ($tool->id === $id) {
					$missing[] = $tool;
			...
			<div class="navigation-container">
				<div class="navigation">
					<div class="nav nav-pills">
						...
						<button class="nav-link" data-bs-toggle="pill" data-bs-target="#tab-tools" type="button">
				...
				<div class="saveable tab-content">
					...
					<div class="card mb-4 tab-pane show active" id="tab-content">
						<div class="card-body">
							...
							<div class="mb-3">
								...
								<div class="form-text">This content will show at the top of the page.</div>
								<?php foreach ($pages as $locale => $p) { ?>
									<div class="language-specific" data-locale="<?php echo $locale; ?>">
					...
					<div class="card mb-4 tab-pane" id="tab-tools">
						<?php
						if (count($missing) > 0) {
						?>
						<div class="card mb-4" id="new">
							<div class="card-header">
								New tools
							</div>
							<div class="card-body">
								<p>These tools were found but haven't been added to the tools page yet. Drag these icons to a category below to add them.</p>

								<div class="tiles" style="margin: 15px 0 0;">
								<?php
								foreach ($missing as $m) {
								?>
								<div class="tile" data-cat="" data-tileid="" data-pos="" data-m-toolid="<?php echo $m->id; ?>" data-m-toolicon="../../../<?php echo $m->getPage('')->getIcon(); ?>" data-m-toolname="<?php echo $m->name; ?>">
									<div class="tool-item" data-tool="<?php echo $m->id; ?>">
										<img src="../../../<?php echo $m->getPage('')->getIcon(); ?>">
...
<script type="text/javascript">
    ...
    <?php
    foreach ($catData as $i => $catName) {
		$translations = [];
		$descriptions = [];

		foreach ($languages as $locale => $name) {
			$target = $studio->basedir . "/resources/languages/$locale/categories.json";
			$lang = json_decode(file_get_contents($target), true);

			if (isset($lang['@' . $catName])) {
				$translations[$locale] = $lang['@' . $catName];
			}
			else {
				$translations[$locale] = 'Untitled';
			}

			$descriptions[$locale] = '';
			if (isset($catDescriptions[$catName])) {
				if (isset($catDescriptions[$catName][$locale])) {
					$descriptions[$locale] = $catDescriptions[$catName][$locale];
				}
			}
		}
        ...
        echo "categories[$i] = {
		token: \"" . sanitize_attribute($catName) . "\",
		names: $translations,
            ...
            foreach ($data as $category => $tools) {
                ...
                foreach ($tools as $id) {
                    ...
                    foreach ($list as $t) if ($t->id == $id) {
                        $icon = '../../../' . $t->getPage('')->getIcon();
                        $name = $t->name;
                    }

                    if ($icon != "") echo "            [\"$id\", \"$icon\", \"$name\", true]," . PHP_EOL;
                }
                echo "        ]," . PHP_EOL . "        ";
            }
        ?>
...
<script src="../../../resources/scripts/tool-editor.js?v=<?php echo PlatformMeta::VERSION; ?>"></script>
