...
$privacyFile = $basedir . DIRECTORY_SEPARATOR . 'privacy.html';
$privacyPublicFile = $studio->basedir . DIRECTORY_SEPARATOR . 'privacy.php';
...
$privacy = file_exists($privacyFile) ? file_get_contents($privacyFile) : '';
$errors = [];
...
checkFile($privacyFile);
checkFile($privacyPublicFile);
...
if (isset($_POST['isPostback']) && file_exists($basedir) && !DEMO) {
	$privacyHTML = trim($_POST['privacy']);

	$success = saveFile($privacyFile, $privacyPublicFile, $privacyHTML, "Privacy Policy");
    $bools = array(
	...
	if ($success) {
		header("Location: privacy.php?success=1");
		die;
	}
}
		...
		<div class="save-container">
			<div class="saveable">
				<div class="panel v2" style="padding: 0;">
					...
					<textarea class="ckeditor" id="privacy" name="privacy"><?php echo $privacy; ?></textarea>
